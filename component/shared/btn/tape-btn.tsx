...
const TapeBtn: FC<TapeBtnProps> = ({ href, label, className, index = 2, disabled, type = 'submit', isLink = false, onClick, textSize, icon, isPending }) => {
 ...
 const renderPendingState = isPending ? ( <EllipsisLoader /> ) : ( <Fragment> {label} {icon} </Fragment> )
  ...
  return (
    <button disabled={disabled} type={type} className={cn(`w-full xs:text-lg permanent-marker-regular text-black font-bold transform flex justify-center -rotate-1 hover:rotate-0 transition-transform`, randomizedClasses[index], textSize ? textSize : 'text-4xl',  className)} onClick={onClick}>
