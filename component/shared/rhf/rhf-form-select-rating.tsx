...
import { ProductRating } from 'component/module'
...
const RHFFormSelectRating = <TSchema extends ZodSchema, TName extends Path<z.infer<TSchema>>>({ control, name, formKey,  options, icon, disabled = false,  className, defaultOption }: RHFFormSelectProps<TSchema, TName>) => {
          ...
          <SelectContent>
            {Array.isArray(options) && options.map((option : string, index: number) => (
              <SelectItem key={index} value={option}>
               <div className={'relative py-0 text-center items-center flex'}>{option.toUpperCase()}&nbsp;<span>{icon && ( <Fragment><ProductRating value={Number(option)} /></Fragment>)}</span></div>
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
...
export default RHFFormSelectRating
